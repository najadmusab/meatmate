...
function loadBeefGallery() {
  ...
  galleryGrid.innerHTML = `<p class="loading">Loading beef listings...</p>`;
    ...
    if (!data) {
      galleryGrid.innerHTML = `
        <div class="empty-state">
          ...
          <p>No beef uploaded yet.</p>
        </div>`;
      return;
    }

    const images = Object.values(data)
      .filter((img) => img.category === "beef")
      .sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));

    renderBeefGallery(images);
  });
}

// 🟣 Render beef cards
function renderBeefGallery(images) {
  const galleryGrid = document.getElementById("galleryGrid");
    ...
    .map((img, i) => {
      ...
      return `
        <div class="image-card">
          <img src="${img.image}" alt="${img.fishName || "Beef"}" />
          <div class="image-info">
            <h3>${img.fishName || "Fresh Beef"}</h3>
            ...
            <div class="button-group">
              ...
              <button class="btn-order" onclick="openOrderForm(${i}, '${(img.fishName || "Beef").replace(/'/g, "\\'")}', '${(img.shopName || "").replace(/'/g, "\\'")}', '${(img.location || "").replace(/'/g, "\\'")}', '${price}')">🐄 Order Now</button>
            </div>
          </div>
        </div>`;
    })
...
function openOrderForm(index, beefName, shopName, location, price) {
  ...
  popup.innerHTML = `
    <div style="background:#fff; padding:20px 24px; border-radius:12px; width:90%; max-width:400px; box-shadow:0 8px 24px rgba(0,0,0,0.2); font-family:Poppins;">
      ...
      <p><strong>Item:</strong> ${beefName}</p>
      <p><strong>Price per kg:</strong> ₹${price}</p>
      ...
      <button id="confirmOrder" style="background:#c0392b;color:white;border:none;padding:10px 14px;border-radius:8px;width:100%;font-weight:600;cursor:pointer;">Confirm Order</button>
